...
\begin{abstract}
  This report presents a hardware profiling analysis for a pipeline that is iterated on to progressively offload networking-intensive tasks from the CPU to a ConnectX-7 BlueField-3 DPU and GPU. The goal is to empirically demonstrate that doing so minimizes latency in the pipeline without sacrificing throughput. 
\end{abstract}
